...
class MainContent extends React.Component {
  ...
  componentDidMount() {
    // console.log(Domain);
    axios
      .get(`https://niceone.alameeremlak.com/api/sliders`, {
        headers: {
          "Accept-Language": "ar",
      ...
      .then((res) => {
        ...
        console.log(images);
        //"https://d3e7ardzpaj3y4.cloudfront.net/image/catalog%2Fapp_component%2F20502116_2021eng-1242x810.gif"
      });
  }
  loopForImg = (showOrder) => {
    for (var i = 0; i < this.state.images.length; i++) {
      if (this.state.images[i].show_order === showOrder) {
    ...
    return (
      <div className="mt-3 mb-5 ">
        <LazyLoad height={100}>
          <Image
            className=" w-100"
            src={config.SERVER_URL + this.loopForImg(1)}
            fluid
          />
        </LazyLoad>
        <LazyLoad height={100}>
          <div className="mt-3 mb-3 ">
            <Image
              className=" w-100"
              src={config.SERVER_URL + this.loopForImg(2)}
              fluid
            />
          </div>
        </LazyLoad>
